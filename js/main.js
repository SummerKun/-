...
$(function () {
  ...
  $('.cancel-editor').click(function () {
    ...
    layer.msg(str, {time:1000,icon:2}, function () {
      $('#mn-editor').find('input').each(function (index, ele) {
        $(ele).val('');
      });
    });
...
$(function () {
  ...
  var initDate = {
    ...
    myMsgDate: function () {
      // 选项卡1中的tbody下的HTML片段
      var htmlDate = '<tr><td><input type="checkbox"></td><td>张三</td><td>园区管理</td><td>2018/1/30 08:00</td><td>' +
        '<button type="button" class="btn btn-primary btn-sm examine"><i class="glyphicon glyphicon-search">' +
        '</i> 查看</button>&nbsp;<button type="button" class="btn btn-danger btn-sm msg-delete-one"  >' +
        '<i class="fa fa-remove"></i> 删除 </button></td></tr>';

      var myMsgNode = $("#my-msg tbody"); //选项卡1中的tbody下的HTML片段
      for (var i = 0; i < 6; i++) {
        myMsgNode.append(htmlDate);
      }
    }
  }
  initDate.myMsgDate();
  ...
  /*$('.examine').on('click', function () {
   ...
   area: ['420px', '240px'], //宽高
   content: '发件人：蓝色爱情海 <746812334@qq.com>'                           +
   '时   间：2017年6月22日(星期四) 中午12:49'+'收件人：李四'+ '借鉴我国民办幼儿园发展中普遍存在的问题，如投资者行为不当、管理系统不实用，管理机制混乱等，' +
   '总结反思作为管理者，如何站在民办幼儿园管理的制高点，尽量少走弯路，' +
   '努力探索适合本地区土壤的民办幼儿园的管理模式，从而保证幼儿园健康、有序的发展是我们需要持续努力的方向与关键。'
   });
   });*/
  ...
  $('#sent-msg-add').on('click', function () {
    ...
    $.each(sendArray, function (index, value) {
      value.toString();
    });

    var $msgForm = $("#sent-msg-addbox");
    var $srecipients = $msgForm.find("input[name=s-recipients]");
    var $stheme = $msgForm.find("input[name=s-theme]");
    var $sconten = $msgForm.find("textarea[name=s-conten]");
    ...
    layer.open({
      ...
      btn1: function (index, layero) {
        ...
        // 删除$("#sent-msg tbody")元素下的".send-delete-one"元素的单机事件
      ...
      btn2: function (index, layero) {
        var htmlDate = '<tr><td><input type="checkbox"></td><td>张三</td><td>园区管理</td><td>2018/1/30 08:00</td><td>' +
          '<button type="button" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-search">' +
          '</i> 查看</button>&nbsp;' +
          '<button class="btn btn-primary btn-sm " data-toggle="modal" data-target="#myModal">' +
          '<i class="fa fa-pencil-square"></i> 编辑 </button>&nbsp;' +
          '<button type="button" class="btn btn-danger btn-sm drafts-delete-one" >' +
          '<i class="fa fa-remove"></i> 删除 </button></td></tr>';

        $("#drafts tbody").off("click", ".drafts-delete-one");

        $("#drafts tbody").on('click', '.drafts-delete-one', function () {
          var node = $(this).parents("tr");
          layer.alert('确认删除该记录吗？', {
            icon: 2,
            title: '删除确认',
        ...
        $("#drafts tbody").append(htmlDate); //选项卡3中的tbody下的HTML片段
        layer.close(index);
        layer.msg('存为草稿成功')
        return false;
      },
